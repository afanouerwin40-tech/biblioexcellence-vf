...
<div class="flex gap-3 mb-6">
    <a href="{{ route('librarian.loans.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium px-5 py-2.5 rounded-xl text-sm transition shadow-lg shadow-blue-200 flex items-center gap-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        Nouvel emprunt
    </a>
    <a href="{{ route('librarian.returns.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-medium px-5 py-2.5 rounded-xl text-sm transition flex items-center gap-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    @if($loans->isEmpty())
    <div class="text-center py-16 text-gray-400 text-sm">Aucun emprunt en cours</div>
    @else
        ...
        <div class="inline-block min-w-full align-middle px-4 lg:px-6">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    $isLate = $dateRetour->lt($now);
                    $diffDays = (int) $dateRetour->diffInDays($now);
                    $daysLate = $isLate ? $diffDays : 0;
                    $daysRemaining = !$isLate ? $diffDays : 0;
                    $colors = ['actif' => 'bg-blue-50 text-blue-600', 'en_retard' => 'bg-red-50 text-red-600', 'renouvele' => 'bg-amber-50 text-amber-600'];
                    @endphp
                    <tr class="hover:bg-gray-50 transition {{ $isLate ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            <p class="text-sm text-gray-700 max-w-xs truncate">{{ $loan->bookCopy->book->titre ?? '—' }}</p>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <p class="text-sm {{ $isLate ? 'text-red-500 font-medium' : 'text-gray-600' }}">{{ $dateRetour->format('d/m/Y') }}</p>
                            @if($isLate)
                            <p class="text-xs text-red-400">{{ $daysLate }} jour{{ $daysLate > 1 ? 's' : '' }} de retard</p>
                            @elseif($daysRemaining <= 3)
                                <p class="text-xs text-amber-400">{{ $daysRemaining }} jour{{ $daysRemaining > 1 ? 's' : '' }} restant{{ $daysRemaining > 1 ? 's' : '' }}</p>
                                @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('librarian.loans.show', $loan) }}" class="text-xs px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200">Détails</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
