    ...
    <div wire:loading.remove class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden
            ...
            <div class="px-4 pb-4 flex items-center gap-2">
                {{-- Bouton Détails --}}
                <a href="{{ route('catalogue.show', $book->id) }}"
                    class="flex-1 text-xs py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg transition font-medium text-center">
                    Détails
                </a>
                ...
                $route = $roleType === 'student'
                ? 'student.reservations.store'
                : 'teacher.reservations.store';
                @endphp
                <form method="POST" action="{{ route($route) }}" class="flex-1">
                    @csrf
                    <input type="hidden" name="book_id" value="{{ $book->id }}">
                    <button type="submit"
                        class="w-full text-xs py-2 bg-amber-50 hover:bg-amber-100
                                               text-amber-600 rounded-lg transition font-medium">
                        Réserver
                    </button>
                </form>
                @endif
